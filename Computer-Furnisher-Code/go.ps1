...
Function Validate-Paths-Exist {
    ...
    Validate-Path-Exists $env:JobFolder "Job Folder"
...
Function Map-Network-Drive {
    New-PSDrive -Name "$env:SharedDriveLetter" -PSProvider "FileSystem" -Root "$env:SharedDriveAddress" | Out-Null
}
...
Function Determine-Job-Name {
    ...
    if ($env:silent -eq "true") {
        Write-Host "Detected existing job '$jobFileName', re-running this job..."
        Validate-Path-Exists "$env:JobFolder/$jobFileName" $jobFileName
        return
    }
    if ($jobFileName -ne ""){ # if job file defined
        $path = "$env:JobFolder/$jobFileName"
        ...
        $res = Read-Host "This process has already been run with the job $jobFileName. Hit E to edit the job, hit C to change or create job, hit Enter to continue with selected job"
        if($res.ToLower() -eq "c") {
            $global:status.JobFileName = ""
            Determine-Job-Name
        }
        if($res.ToLower() -eq "e") {
            Notepad.exe "$env:JobFolder/$jobFileName" | Out-Null #Out-null causes powershell to wait till process killed
        }
        return
    }
    ...
    if ($res.ToLower() -eq "n") {
        Validate-Path-Exists "$env:JobFolder/jobTemplate.xml" "JobTemplate.xml"
        $jobName = Read-Host "Please specify new job name"
        $jobName  = $jobName -replace '.xml',''
        Write-Host "Creating..."
        Copy-Item "$env:JobFolder/jobTemplate.xml" "$env:JobFolder/$jobName.xml"
        $global:status.JobFileName = "$jobName.xml"
        $global:status.NumberOfExecutions = 0
    } else {
        $res  = $res -replace '.xml',''
        $path = "$env:JobFolder/$res.xml"
        $pathExists = Test-Path $path
        if ($pathExists -eq $FALSE) {
...
Function Add-New-Tasks-To-Jobs () {
    Get-ChildItem $env:JobFolder -Filter *.xml | 
    Foreach-Object {
        $fileName = $_.Name
        $jobFromFile = Load-Job-From-File $env:JobFolder $fileName
        Foreach ($task in $global:tasks) {
            if ($jobFromFile.Tasks.ContainsKey($task.identifier) -eq $FALSE) {
                $jobFromFile.Tasks[$task.identifier] = $FALSE
                Upsert-File-From-Object $jobFromFile "$env:JobFolder/$fileName"
            }
        }
    }
}
...
Function Setup-Auto-Restart() {
    $val = Get-ItemProperty -Path "hklm:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name "AutoAdminLogon"
    if($val.AutoAdminLogon -ne 1)
    {
        $creds = $NULL
        $creds = Get-Credential
        Write-Host "Enabling autologon as administrator. The computer will restart into "
        & "./executables/autologon/autologon.exe" "Administrator" $env:computername $creds.GetNetworkCredential().Password 
    }
    Write-Host "Computer already configured to restart as admin."
}

Function Restart-If-Required(){
    #don't run setupauto restart if finished
    # If > 3 executed and all success
...
Determine-Job-Name
$global:job = Load-Job-From-File $env:JobFolder $global:status.JobFileName
...
if ($computerFurnisherScheduledTasks -eq $NULL -or $computerFurnisherScheduledTasks.count -eq 0) {
    ...
    schtasks /Create /XML $env:LocalComputerFurnisherScheduledTaskPath /TN "Computer Furnisher"
...
if ($global:useStubs -eq $TRUE) {
    Read-Host "Done... PAUSED"
}